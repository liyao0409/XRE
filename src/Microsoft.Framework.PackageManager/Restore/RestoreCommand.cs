...
namespace Microsoft.Framework.PackageManager
{
    public class RestoreCommand
    {
        ...
        public async Task<bool> ExecuteCommand()
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(GlobalJsonFile))
                {
                    ...
                    await ForEachAsync(
                        projectJsonFiles,
                        maxDegreesOfConcurrency: Environment.ProcessorCount * 2,
                        body: async projectJsonPath =>
                        {
        ...
        private static Task ForEachAsync<TVal>(IEnumerable<TVal> source,
                                               ...
                                               Func<TVal, Task> body)
        {
            var tasks = from partition in Partitioner.Create(source).GetPartitions(maxDegreesOfConcurrency)
